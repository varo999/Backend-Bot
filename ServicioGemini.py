...
class ServicioGemini:
    ...
    def hacer_pregunta(self, pregunta: str, historial: List[Tuple[str, str]], id_almacen: str, modelo: str = "gemini-2.5-flash", top_k: int = 3):
        """
        Realiza una pregunta a Gemini indicando específicamente en qué almacén buscar.
        """
        # 1. Asegurar conexión
        ...
        # 2. Validar que nos han pasado un almacén para esta pregunta
        if not id_almacen:
                    ...
                    tools=[
                        types.Tool(
                            file_search=types.FileSearch(
                                # Aquí usamos el ID dinámico que viene por parámetro
